...
namespace URLShortener.Infrastructure.Services.URL
{
    public class URLService : IURLService
    {
        ...
        private async Task IsPurchasePossible(int userID)
        {
            ...
            double balance = (await _userRepository.GetAsync(x => x.ID == userID, i => i.Include(i => i.FinancialRecord), null, asNoTracking: true)
        ...
        private async Task<List<URLCategoryModel>> ResolveOrCreateCategory(string categoryString, UserModel user)
        {
            ...
            foreach (var categoryName in categories)
            {
                ...
                URLCategoryModel? category = await _urlCategoryRepository
                                                .GetAsync(c => c.Title == categoryName && c.UserID == user.ID);
                ...
                if (category == null)
                {
                    ...
                    _urlCategoryRepository.Add(category);
                }
                result.Add(category);
            }
            return result;
        ...
        private async Task<URLModel?> IsURLAlreadyAsync(string longURL, int userID)
        {
            ...
            URLModel? url = await _urlRepository.GetAsync(x => x.LongURL == longURL && x.UserID == userID, i => i.Include(r => r.Categories));
            return url;
        }
        ...
        private async Task<List<URLModel>> IsURLAlreadyAsync(HashSet<string> longURLs, int userID)
        {
            ...
            List<URLModel> URLs = await _urlRepository.Query().Include(x => x.Categories).AsNoTracking().Where(x => longURLs.Contains(x.LongURL) && x.UserID == userID).ToListAsync();
            return URLs;
        }
        ...
        {
            if (await _urlRepository.AnyAsync(x => x.ShortCode == shortcode))
