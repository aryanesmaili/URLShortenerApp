...
namespace URLShortener.Infrastructure.Services.User
{
                               ...
                               IUnitOfWork uow) : IUserService
    {
        ...
        {
            List<URLDTO> urls = await _urlRepository
        ...
        private async Task<List<URLDTO>?> GetRecentURLs(int userID)
        {
            ...
            List<URLDTO> recentURLs = await _urlRepository
                .Query(asNoTracking: true)
                .Where(x => x.UserID == userID)
        ...
        private async Task<List<string>?> GetTopCountries(int userID)
        {
            ...
            List<LocationInfo?> countries = await _clickRepository
                .Query()
                .Include(x => x.URL)
                .AsNoTracking()
        ...
        private async Task<List<string>?> GetTopDeviceOS(int userID)
        {
            ...
            List<DeviceInfo?> topDevices = await _clickRepository
                .Query()
                .Include(x => x.URL)
                .AsNoTracking()
        ...
        private async Task<Dictionary<string, int>?> GetClicksInDay(int userID)
        {
            ...
            var clickCounts = await _clickRepository.Query()
                .Include(x => x.URL)
                .AsNoTracking()
                .Where(x => x.URL.UserID == userID)
                .ToListAsync();
        ...
        private async Task<Dictionary<string, int>?> GetClicksInMonth(int userID)
        {
            ...
            var clickCounts = await _clickRepository
                .Query()
                .Include(x => x.URL)
                .AsNoTracking()
                .Where(x => x.URL.UserID == userID &&
                            x.ClickedAt.Month == currentDate.Month &&
                            x.ClickedAt.Year == currentDate.Year)
                .ToListAsync();
